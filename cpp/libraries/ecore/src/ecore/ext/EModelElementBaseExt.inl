...
namespace ecore::ext
{
    ...
    std::shared_ptr<EAnnotation> EModelElementBaseExt<I...>::getEAnnotation( const std::string& source )
    {
        ...
        {
            auto it
                = std::find_if( eAnnotations_->begin(), eAnnotations_->end(), [=]( const auto& a ) { return source == a->getSource(); } );
            ...
            if( firstCharacter != '@' )
            {
                ...
                if( firstCharacter == '%' )
                {
                    ...
                    if( index != 0 && ( index == uriFragmentSegment.size() - 1 || ( hasCount = uriFragmentSegment[index + 1] == '.' ) ) )
                    {
                        ...
                        auto eAnnotationClass = EcorePackage::eInstance()->getEAnnotation();
                        auto contents = eContents();
                        for( auto eObject : *contents )
                        {
                            if( EcoreUtils::isAssignableFrom( eAnnotationClass, eObject->eClass() ) )
                            {
                                auto eAnnotation = std::static_pointer_cast<EAnnotation>( eObject );
                ...
                auto eNamedElementClass = EcorePackage::eInstance()->getENamedElement();
                auto contents = eContents();
                for( auto eObject : *contents )
                {
                    if( EcoreUtils::isAssignableFrom( eNamedElementClass, eObject->eClass() ) )
                    {
                        auto eNamedElement = std::static_pointer_cast<ENamedElement>( eObject );
    ...
    template <typename... I>
    std::string EModelElementBaseExt<I...>::eURIFragmentSegment( const std::shared_ptr<EStructuralFeature>& feature,
                                                                 const std::shared_ptr<EObject>& eObject ) const
    {
        ...
        if( EcoreUtils::isAssignableFrom( eNamedElementClass, eObject->eClass() ) )
        {
            ...
            for( auto otherEObject : *contents )
            {
                ...
                if( EcoreUtils::isAssignableFrom( eNamedElementClass, otherEObject->eClass() ) )
                {
                    auto otherENamedElement = std::static_pointer_cast<ENamedElement>( otherEObject );
        ...
        if( EcoreUtils::isAssignableFrom( eAnnotationClass, eObject->eClass() ) )
        {
            ...
            for( auto otherEObject : *contents )
            {
                ...
                if( EcoreUtils::isAssignableFrom( eAnnotationClass, otherEObject->eClass() ) )
                {
                    auto otherEAnnotation = std::static_pointer_cast<EAnnotation>( otherEObject );
